...
export default function AprovacoesClient({ submissions: initial, reviewerId }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        {queue.length === 0 ? (
          ...
          queue.map(sub => (
            <div key={sub.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              {sub.proof_url && (
                <div className="px-4 py-3 border-b border-gray-100">
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Comprovante</p>
                  <button
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-5 py-2.5 rounded-full text-sm font-medium shadow-lg z-50">
          {toast}
        </div>
      )}
    </div>
